...
class Hashing:
    ...
    def encoding(self, NewPassword):
        key = hashlib.pbkdf2_hmac(
            "sha512", NewPassword.encode("utf-8"), self.__Salt, 100_000
        )
        return str(key)
...
class User(Hashing):
    ...
    def readUserDetail(self):
        try:
            MyFile = open("Users_Detail.txt", "r")
        except:
            raise FileExistsError("File has not been found!")
        for i in MyFile:
            ...
            raise BaseException(
                " This user does not exist on the << Users_Detail.txt>> file"
            )
        MyFile.close()
...
class SignUp(User):
    ...
    def writeInTheFile(self):
        try:
            with open("Users_Pass.txt", "a") as MyFile:
        ...
        try:
            ...
            with open("Users_Detail.txt", "a") as MyFile:
